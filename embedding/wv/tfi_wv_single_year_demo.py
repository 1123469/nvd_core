...
def get_train_and_label_dataset(year,ig_num):
    label_path = '..\\..\\data\\clean\\nvdcve-1.1-' + year + '_labels.csv'
    wv_model_path = '..//..//models//wv//' + year + "_" + str(vec_len) + "_" + str(min_count) + "_" + str(
        window_len) + '.pkl'
    ig_path = '..//..//data//ig//'+year+"_"+str(ig_num)
    ...
    for line in dataset['contents']:
        ...
        if length > n:
            ...
            weights = np.eye(n)
            for i in range(n):
                if line[i] in gain_dict:
                    weights[i] = np.dot(np.exp(gain_dict[line[i]]), weights[i])
            word2vec_matrix = np.matmul(weights, wv_model.wv[line])
...
if __name__ == '__main__':
    train_dataset,label_dataset = get_train_and_label_dataset(year,ig_num)
    ...
    cnn_model.compile(optimizer=tf.optimizers.Adam(1e-3),
    ...
    # cnn_model.fit(x=x_train, y=y_train, epochs=10, verbose=2)
